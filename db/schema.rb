# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# This file is the source Rails uses to define your schema when running `bin/rails
# db:schema:load`. When creating a new database, `bin/rails db:schema:load` tends to
# be faster and is potentially less error prone than running all of your
# migrations from scratch. Old migrations may fail to apply correctly if those
# migrations use external dependencies or application code.
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema[8.1].define(version: 2026_05_28_191419) do
  # These are extensions that must be enabled in order to support this database
  enable_extension "pg_catalog.plpgsql"

  create_table "day_credits", force: :cascade do |t|
    t.datetime "created_at", null: false
    t.date "credit_date", null: false
    t.string "credit_type", default: "bank_holiday", null: false
    t.integer "credited_minutes", null: false
    t.text "note"
    t.datetime "updated_at", null: false
    t.bigint "user_id", null: false
    t.index ["user_id", "credit_date"], name: "index_day_credits_on_user_id_and_credit_date"
    t.index ["user_id"], name: "index_day_credits_on_user_id"
  end

  create_table "sessions", force: :cascade do |t|
    t.datetime "created_at", null: false
    t.string "ip_address"
    t.datetime "updated_at", null: false
    t.string "user_agent"
    t.bigint "user_id", null: false
    t.index ["user_id"], name: "index_sessions_on_user_id"
  end

  create_table "time_entries", force: :cascade do |t|
    t.datetime "clock_in"
    t.datetime "clock_out"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.bigint "user_id", null: false
    t.index ["user_id"], name: "index_time_entries_on_user_id"
  end

  create_table "time_entry_breaks", force: :cascade do |t|
    t.datetime "break_in"
    t.datetime "break_out"
    t.datetime "created_at", null: false
    t.string "reason"
    t.bigint "time_entry_id", null: false
    t.datetime "updated_at", null: false
    t.index ["time_entry_id"], name: "index_time_entry_breaks_on_time_entry_id"
  end

  create_table "users", force: :cascade do |t|
    t.boolean "admin", default: false, null: false
    t.integer "contracted_hours", default: 37
    t.datetime "created_at", null: false
    t.string "email", null: false
    t.string "password_digest", null: false
    t.datetime "updated_at", null: false
    t.boolean "verified", default: false, null: false
    t.integer "working_days_per_week", default: 5, null: false
    t.index ["email"], name: "index_users_on_email", unique: true
  end

  create_table "week_entries", force: :cascade do |t|
    t.datetime "beginning_of_week"
    t.integer "offset_in_minutes"
    t.integer "required_minutes", default: 0, null: false
    t.bigint "user_id", null: false
    t.index ["user_id"], name: "index_week_entries_on_user_id"
  end

  add_foreign_key "day_credits", "users"
  add_foreign_key "sessions", "users"
  add_foreign_key "time_entries", "users"
  add_foreign_key "time_entry_breaks", "time_entries"
  add_foreign_key "week_entries", "users"
end
